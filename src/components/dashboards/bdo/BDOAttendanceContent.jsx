...
const BDOAttendanceContent = () => {
  // BDO: Location state management via BDOLocationContext
  const {
    activeScope,
    selectedLocation,
    selectedLocationId,
  ...
  const selectedDistrictForHierarchy = useMemo(
    () => bdoDistrictId ? { id: bdoDistrictId, name: bdoDistrictName } : null,
    [bdoDistrictId, bdoDistrictName]
  );
  const selectedBlockForHierarchy = useMemo(
    () => bdoBlockId ? { id: bdoBlockId, name: bdoBlockName } : null,
    [bdoBlockId, bdoBlockName]
  );
  const setSelectedDistrictId = () => {}; // No-op for BDO
  ...
  // Top 3 section state
  const [top3Scope, setTop3Scope] = useState('Block'); // BDO: Default to Block (no District)
  ...
  const handleScopeChange = async (scope) => {
    ...
    if (scope === 'State') {
      ...
      // BDO: For Blocks tab, show "Select Block"
      console.log('🔄 BDO: Scope changed to Blocks');
      setSelectedLocation('Select Block');
      setDropdownLevel('blocks');
      contextTrackTabChange(scope);
    } else if (scope === 'GPs') {
      // BDO: For GPs tab, show "Select GP"
      console.log('🔄 BDO: Scope changed to GPs');
  ...
  useEffect(() => {
    // BDO: Auto-fetch blocks using bdoDistrictId
    if (bdoDistrictId) {
      console.log('🔄 BDO: Auto-fetching blocks for district:', bdoDistrictId);
...
export default BDOAttendanceContent;