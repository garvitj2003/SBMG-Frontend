...
const CEOVillageMasterContent = () => {
  ...
  const scopeButtons = ['Districts', 'Blocks', 'GPs']; // CEO can only view Blocks and GPs
  ...
  useEffect(() => {
    if (!selectedDistrictId || !selectedFyId) return;

    console.log("🚀 Auto fetching CEO district analytics");

    fetchAnalytics();

  }, [selectedDistrictId, selectedFyId]);
    ...
    try {
      ...
      // Build URL based on active scope
        ...
        if (selectedBlockId) {
          ...
          // District overview (default for CEO)
          url = `/annual-surveys/analytics/district/${selectedDistrictId}?fy_id=${fyId}`;
        }
      }
  ...
  // For auto block first Select
  useEffect(() => {
    if (activeScope === 'Blocks' && blocks.length > 0 && !selectedBlockId) {

      const firstBlock = blocks.find(
        b => b.district_id === selectedDistrictId
      );
      ...
      if (firstBlock) {
        ...
        setSelectedBlockForHierarchy(firstBlock);
      }
    }
  }, [activeScope, blocks]);
  ...
  useEffect(() => {
    if (
      ...
      if (firstBlock) {
        ...
        fetchGramPanchayats(selectedDistrictId, firstBlock.id);
      }
    }
  }, [activeScope, blocks]);
  ...
  useEffect(() => {
    if (
      ...
      console.log("📡 Fetching GP Surveys for:", selectedGPId);
      fetchGpSurveys();
    }
  }, [activeScope, selectedGPId, selectedFyId]);
  ...
  useEffect(() => {
    if (
      ...
      const firstGP = gramPanchayats.find(
        gp => gp.block_id === selectedBlockForHierarchy.id
      );
  ...
  const handleScopeChange = (scope) => {
    ...
    if (scope === 'Blocks') {
      updateLocationSelection('Blocks', 'Select Block', null, selectedDistrictId, null, null, 'tab_change');
    }
  ...
  useEffect(() => {
    if (!loadingCEOData && ceoDistrictId && ceoDistrictName) {

      console.log("🎯 Force setting CEO district as default");
  ...
  }, [loadingCEOData, ceoDistrictId]);


  // Fetch GP surveys when in GP scope with selected GP and FY
  useEffect(() => {

    if (!selectedFyId || !selectedDistrictId) return;


    console.log("📡 Triggering analytics for scope:", activeScope);
  ...
  // console.log("ceoDistrictNamee:", ceoDistrictName);
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F3F4F6' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          >
            <button
              onClick={() => activeScope !== 'Districts' && setShowLocationDropdown(!showLocationDropdown)}
              disabled={activeScope === 'Districts'}
              style={{
                width: '100%',
                padding: '5px 12px',
                border: '1px solid #d1d5db',
                borderRadius: '10px',
                backgroundColor: activeScope === 'Districts' ? '#f9fafb' : 'white',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
                cursor: activeScope === 'Districts' ? 'not-allowed' : 'pointer',
                fontSize: '14px',
                color: activeScope === 'Districts' ? '#9ca3af' : '#6b7280',
                opacity: activeScope === 'Districts' ? 0.6 : 1
            ...
            >
              ...
              <ChevronDown style={{
                width: '16px',
                height: '16px',
                color: activeScope === 'Districts' ? '#d1d5db' : '#9ca3af'
              }} />
            </button>
            ...
            {showLocationDropdown && activeScope !== 'State' && (
              <div
                ...
                style={{
                  ...
                  backgroundColor: activeScope === 'Districts' ? '#f3f4f6' : 'white',
                  cursor: activeScope === 'Districts' ? 'not-allowed' : 'pointer',
                }}
              >
