...
const VDOGpsTrackingContent = () => {
    ...
    const selectedLocation = vdoLocation?.getCurrentLocationInfo() || {};
    ...
    // VDO has no scope selector - only sees their assigned GP
    
    // Fetch vehicles from API based on VDO's location context
    const { data: vehiclesData = [], isLoading: isLoadingVehicles, error: vehiclesError } = useVehicles({
        districtId: selectedLocation?.districtId || selectedLocation?.vdoDistrictId || 1,
        blockId: selectedLocation?.blockId || selectedLocation?.vdoBlockId || 1,
        gpId: selectedLocation?.gpId || selectedLocation?.vdoGPId || 1,
    });
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#F3F4F6' }}>
            ...
            <div style={{
                ...
                {/* Right side - Location display (no scope selector for VDO) */}
                <div style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '16px'
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <span>{selectedLocation?.gp || 'No location selected'}</span>
                        </div>
                    </div>
                </div>
            </div>
