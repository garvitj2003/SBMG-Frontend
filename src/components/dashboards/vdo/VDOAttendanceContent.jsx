...
const VDOAttendanceContent = () => {
  ...
  const selectedDistrictForHierarchy = useMemo(
    () => vdoDistrictId ? { id: vdoDistrictId, name: vdoDistrictName } : null,
    [vdoDistrictId, vdoDistrictName]
  );
  const selectedBlockForHierarchy = useMemo(
    () => vdoBlockId ? { id: vdoBlockId, name: vdoBlockName } : null,
    [vdoBlockId, vdoBlockName]
  );
  const selectedGPForHierarchy = useMemo(
    () => vdoGPId ? { id: vdoGPId, name: vdoGPName } : null,
    [vdoGPId, vdoGPName]
  );
  ...
  useEffect(() => {
    // BDO: Auto-fetch blocks using vdoDistrictId
    if (vdoDistrictId) {
      console.log('🔄 BDO: Auto-fetching blocks for district:', vdoDistrictId);
...
export default VDOAttendanceContent;