  ...
  return (
        ...
        >
                ...
                {viewMode === 'sent' && (sentPage > 1 || sentHasMore || sentNotices.length === pageSize) && (() => {
                  const sentTotalPages = sentTotal != null ? Math.ceil(sentTotal / pageSize) : null;
                  const getSentPageNumbers = () => {
                    if (sentTotalPages != null) {
                    ...
                    const p = [1];
                    if (sentPage > 2) p.push('...');
                    if (sentPage > 1) p.push(sentPage);
                    if (sentHasMore) p.push(sentPage + 1);
                    return p;
                  };
                  const btn = (on, dis) => ({
                    onClick: on, disabled: dis,
                    style: { padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '8px', backgroundColor: dis ? '#f9fafb' : 'white', color: dis ? '#9ca3af' : '#374151', cursor: dis ? 'not-allowed' : 'pointer', fontSize: '14px', fontWeight: '500', display: 'inline-flex', alignItems: 'center', gap: '4px' }
                  });
                  return (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', padding: '20px', borderTop: '1px solid #e5e7eb', flexWrap: 'wrap' }}>
                      ...
                      {getSentPageNumbers().map((n, i) => typeof n === 'number' ? (
                      ...
                      <button {...btn(() => setSentPage(prev => prev + 1), (!sentHasMore && sentNotices.length < pageSize) || loadingSent)}>Next</button>
                      {sentTotalPages != null && (
                ...
                {viewMode === 'received' && (receivedPage > 1 || receivedHasMore || receivedNotices.length === pageSize) && (() => {
                  const recvTotalPages = receivedTotal != null ? Math.ceil(receivedTotal / pageSize) : null;
                  const getRecvPageNumbers = () => {
                    if (recvTotalPages != null) {
                    ...
                    const p = [1];
                    if (receivedPage > 2) p.push('...');
                    if (receivedPage > 1) p.push(receivedPage);
                    if (receivedHasMore) p.push(receivedPage + 1);
                    return p;
                  };
                  ...
                  return (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', padding: '20px', borderTop: '1px solid #e5e7eb', flexWrap: 'wrap' }}>
                      ...
                      {getRecvPageNumbers().map((n, i) => typeof n === 'number' ? (
                      ...
                      <button {...btn(() => setReceivedPage(prev => prev + 1), (!receivedHasMore && receivedNotices.length < pageSize) || loadingReceived)}>Next</button>
                      {recvTotalPages != null && (
